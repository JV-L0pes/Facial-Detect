...
class LivenessDetector:
    ...
    def _analyze_movement(self) -> bool:
        ...
        centers = np.array(
            [frame[:2] for frame in self.frame_history[-LIVENESS_FRAMES_REQUIRED:]]
        )
...
class AdvancedLivenessDetector:
    ...
    def add_frame(
        ...
        print(
            f"DEBUG LIVENESS: Adicionando frame - Histórico atual: {len(self.frame_history)}"
        )
        ...
        if len(self.texture_history) >= LIVENESS_FRAMES_REQUIRED:
            print(
                f"DEBUG LIVENESS: Analisando liveness com {len(self.texture_history)} frames"
            )
        ...
        print(
            f"DEBUG LIVENESS: Frames insuficientes ({len(self.texture_history)} < {LIVENESS_FRAMES_REQUIRED})"
        )
        return False
    ...
    def _detect_blink(self):
        ...
        try:
            ...
            if (
                current_ear < self.blink_threshold
                and previous_ear >= self.blink_threshold
            ):
                self.blink_count += 1
    ...
    def get_stats(self) -> dict:
        ...
        texture_variance = (
            np.var(self.texture_history) if len(self.texture_history) > 1 else 0
        )
        ...
        if BLINK_DETECTION_ENABLED:
            blink_passed = (
                self.blink_count > 0
                or len(self.eye_aspect_ratios) < LIVENESS_FRAMES_REQUIRED
            )
