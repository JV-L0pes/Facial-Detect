...
def test_face_recognition():
    ...
    try:
        ...
        if stats["total_embeddings"] == 0:
            print(
                "ERRO: Nenhum embedding encontrado! O sistema nao foi inicializado corretamente."
            )
            return False
...
def test_api_endpoint():
    ...
    try:
        ...
        payload = {"image": test_image_data}

        response = requests.post(
            "http://localhost:8000/api/validate", json=payload, timeout=10
        )
        ...
        print(
            "ERRO: Nao foi possivel conectar a API. Verifique se o backend esta rodando."
        )
        return False
    except Exception as e:
